...
- **ansible-scaffold-role command**: Use `/ansible-scaffold-role` to create a new Ansible role that fully complies with Red Hat CoP good practices. Features include:
  - Interactive variable builder that asks what the role manages (packages, services, configs, users, firewall, storage) and generates realistic defaults, tasks, handlers, and templates
...
- The scaffold command follows an interactive pattern: gather user input about what the role manages, then generates appropriate variables, tasks, handlers, and templates pre-populated with realistic content
